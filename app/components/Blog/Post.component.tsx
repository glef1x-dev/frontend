...
export function _Post({ blogPost, index }: PostProps): JSX.Element {
  const ariaLabel = `Read blog post: ${blogPost.title}`;
  const href = `/blog/${blogPost.slug}`;
    ...
    >
      ...
      <div
        className={clsx(
          ...
          blogPost.image && index <= 2 && "lg:(rounded-tr-none rounded-tl-none)"
        )}
      >
