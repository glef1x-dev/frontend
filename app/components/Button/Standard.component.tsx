...
const ButtonStyles =	'flex justify-center items-center h-12 px-8 py-4 bg-gray-50 hover:(bg-gray-100 bg-opacity-50 text-primary-400) dark:(bg-gray-900 hover:bg-gray-800) text-base font-bold text-primary-300 rounded-lg default-transition default-focus';
...
export function Standard({
  ...
  switch (rest.type) {
    case NavigationItemType.LINK:
      ...
      return (
        <Link {...rest} className={clsx(ButtonStyles, className)} href={rest.href} passHref>
          {icon && <Icon className="mr-2" icon={icon} />}
          {children}
        </Link>
      );
